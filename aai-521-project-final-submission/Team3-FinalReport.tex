...
\usepackage{float}  % For [H] placement - ADDED
...
\title{A Scalable Agentic AI for Multi-Modal Video Classification,Summarization, and
Analysis: A Comprehensive Technical Report}

\author{\IEEEauthorblockN{AAI-521 Advanced Multi-Modal Analysis}
\IEEEauthorblockA{\textit{Gangadhar Singh Shiva,  gshiva@sandiego.edu, Nagarajan Mahalingam nmahalingam@sandiego.edu, Akshobhya Rao BV,  abv@sandiego.edu} \\
\textit{San Deigo University}\\
December 2025}
}
...
\begin{abstract}
This report details the architectural design, implementation, and verification of a \textbf{scalable Agentic AI framework} for comprehensive multi-modal video analysis, encompassing classification, summarization, and external contextualization. The system utilizes a six-step \textbf{multi-modal LangGraph} pipeline that integrates vision, audio, and contextual grounding in real-time. The heart of system is an autonomous workflow that dynamically processes raw video based on the confidence of the initial activity classification. Key components include a \textbf{ResNet18} backbone fine-tuned using \textbf{LoRA (Low-Rank Adaptation)} for parameter-efficient classification, and a \textbf{YOLOv8n} model combined with the \textbf{Structural Similarity Index (SSIM)} for efficient keyframe extraction. Critical logic refinements, such as the normalization of the 'Punch' class to 'boxing' for news queries and the universal news fetching node, validate the pipeline's \textbf{resilience and adaptability}. Verification against multiple test cases confirms that the framework successfully handles both high-confidence and low-confidence inputs, providing comprehensive, context-aware insights while maintaining a focus on computational efficiency.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Ensure Multi-Modal Resilience:} Implement auxiliary data retrieval (audio transcription) and crucial logic refinements (classification normalization) to maintain output quality and guaranty relevant contextual data (news fetching) even for ambiguous or low-confidence inputs.
\end{enumerate}
...
\textbf{ResNet} (Residual Neural Network) is a foundational \textbf{Convolutional Neural Network (CNN)} architecture that solves the problem of \textbf{vanishing gradients} in deep networks. It uses \textbf{residual connections} (skip connections) in which the input $x$ is added to the output of a layer stack $F(x)$, resulting in the output $H(X) = F(X) + X$. This formulation facilitates the learning of an identity mapping, ensuring that deeper layers do not degrade performance but rather allow for continuous feature refinement.

\subsection{Low-Rank Adaptation (LoRA)}
\textbf{LoRA} is a pivotal \textbf{Parameter-Efficient-Fine-Tuning (PEFT)} technique designed to quickly and efficiently adapt large, pretrained models to tasks while minimizing computational resources. For a pretrained weight matrix $W_0$, the update is represented as $W = W_0 + BA$, where $B$ and $A$ are low-rank matrices. By freezing $W_0$ and only training $A$ and $B$, the number of trainable parameters is drastically reduced, enabling high-fidelity adaptation with minimal RAM consumption.

% FIGURE 1 - ResNet18 LoRA Block - placed HERE
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\columnwidth]{resnet18_lora_block.png}
    \caption{\textbf{ResNet18 LoRA Residual Block Architecture.} The diagram illustrates how LoRA adapters (trainable matrices A and B) are injected into the residual block alongside frozen pretrained weights ($W_0$). The output combines both the frozen and adapted pathways: $H(X) = F(X) + X$,  $F(x)$ incorporates the LoRA adaptation $W_0 + BA$.}
...
% FIGURE 2 - HAR Architecture - placed HERE
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\columnwidth]{har_architecture.png}
...
% FIGURE 4 - LangGraph Workflow - placed HERE
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\columnwidth]{langgraph_workflow.png}
...
% FIGURE 5 - Training Curves - placed HERE
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\columnwidth]{training_curves.png}
...
The following results summarize the successful operation of the pipeline across the test cases:

% FIGURE 6 - Summarization Results - placed HERE
\begin{figure*}[!h]
    \centering
    \begin{subfigure}{0.32\textwidth}
        \centering
